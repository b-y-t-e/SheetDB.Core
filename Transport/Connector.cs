    ...
    public class Connector : IConnector
    {
        ...
        private string GetComputedSignature(DateTime now)
        {
            ...
            using (var hashAlg = new SHA256Managed())
            {
                ...
                var signature = Encode.UrlBase64Encode(
                    privateKey.SignData(headerPayloadBytes, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1));
                    //privateKey.SignData(headerPayloadBytes, hashAlg));

                return headerAndPayload + "." + signature;
            }
        }

        private System.Security.Cryptography.RSACng GetPrivateKey()
        {
            var certificate = new X509Certificate2(this._privateKey, "notasecret", X509KeyStorageFlags.Exportable);

            var rsa = (System.Security.Cryptography.RSACng)certificate.PrivateKey;
            return rsa;
